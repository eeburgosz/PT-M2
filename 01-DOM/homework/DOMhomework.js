...
//document.querySelector('#createdBy').append(' Ernesto Burgos');
var span= document.querySelector('#createdBy');
span.innerHTML= span.innerHTML + ' Ernesto Burgos';
...
function buildToDo(todo, index) {  
  ...
  toDoShell.className= 'toDoShell'; // (2) -> Clase 'toDoShell' definida en assets/styles.css
  //------------------Hasta ahora en el HTML------------------//
  // <div class='toDoShell'>
  //   Bla bla bla bla
  // </div>
  //----------------------------------------------------------//
  ...
  if (todo.complete){
    ...
    chkbx.className= 'completeCheckbox';
    chkbx.checked =true;
  } // (6) -> Clase 'completeText' definida en assets/styles.css

  toDoShell.appendChild(toDoText); // (7)
  toDoShell.appendChild(chkbx);
  //------------------Hasta ahora en el HTML------------------//
  // <div class='toDoShell'>
  //   <span id='index' class='completeText' (true/>false)> Tarea por hacer </span>
  // </div>
  //----------------------------------------------------------//

  //------------------Evento para las tareas------------------//
  //toDoText.addEventListener('click', completeToDo);
  chkbx.addEventListener('click', completeToDo);
  //----------------------------------------------------------//
...
function buildToDos(toDos) {
  ...
  // toDoS= [{description, complete}, {description, complete}, {description, complete}] -> Creación de arreglo de los elementos generados en 'buildToDo' 

  /* var _array= toDos.map((todo, i) => {
    return buildToDo(todo, i)
  });
  // _array= [<div class='toDoShell'> <span id='0'> Tarea 1 </span> </div>,
  //          <div class='toDoShell'> <span id='1'> Tarea 2 </span> </div>,
  //          <div class='toDoShell'> <span id='2'> Tarea 3 </span> </div>]
...
function displayToDos() {
  // Tu código acá:
  var toDoContainer= document.querySelector('#toDoContainer'); // (1) -> <div id='toDoContainer'></div>
  toDoContainer.innerHTML= ''; // (2) -> <div id='toDoContainer'>''</div>

  var build= buildToDos(toDoItems); // (3) -> toDoItems= [{description, complete}, {description, complete}, {description, complete}]
  // build= [<div class='toDoShell'> <span id='0'> Tarea 1 </span> </div>,
  //         <div class='toDoShell'> <span id='1'> Tarea 2 </span> </div>,
  //         <div class='toDoShell'> <span id='2'> Tarea 3 </span> </div>]

  // Quiero meter los valores en el paso (2)

  for (let i= 0; i< build.length; i++){
    toDoContainer.appendChild(build[i])
  }
  // <div id='toDoContainer'>
  //  <div class='toDoShell'> <span id='0'> Tarea 1 </span> </div>
  //  <div class='toDoShell'> <span id='1'> Tarea 2 </span> </div>
  //  <div class='toDoShell'> <span id='2'> Tarea 3 </span> </div>
  // </div>
}
...
function addToDo() {
  ...
  let input= document.querySelector('#toDoInput'); // (1) -> en el HTML, es el id que tiene el input
  if (input.value !== ''){
    let toDo = new ToDo (input.value); // (2) toDo = {description: input.value, complete: false}
    toDoItems.push(toDo); // (3) Actualicé el arreglo de ToDos
    input.value= '' // Reinicio el input para que quede vacío
    displayToDos(); // (4) Muestro los nuevos ToDos en pantalla
  } 
...
function completeToDo(event) {
  ...
  const index = event.target.id;
    //event - click
    //event.target - spam
    //event.target.id - index buildTodo -> index del spam
